...
\begin{itemize}
    ...
    \item \textbf{Decomposizione in porzioni ad alta densità (Chunking):}
...
\begin{itemize}
    \item \textbf{Accesso in tempo costante $\mathcal{O}(1)$ durante la DFS:}
          Durante la ricerca in profondità dei percorsi (\cref{sec:impl-routes}),
          l'indicizzazione per chiave di entità consente l'estrazione immediata di
          proprietà e relazioni, evitando la scansione sequenziale di una lista
          globale ($\mathcal{O}(N)$).

    \item \textbf{Gestione della memoria (\texttt{MAX\_STORED\_VALUES}):} Per i
          campi ad alta cardinalità (es. misurazioni continue), il
          \texttt{DiscoveryMixin} applica un limite rigido alla lista \texttt{values},
          memorizzando unicamente un campionario rappresentativo per la successiva
          fase di iniezione.

    \item \textbf{Parallelizzazione dell'I/O
              (\texttt{MAX\_DISCOVERY\_WORKERS}):} Poiché le chiamate REST per la scoperta
          delle singole entità sono operazioni indipendenti, il recupero iniziale
          viene velocizzato sfruttando un pool di thread tramite
          \texttt{concurrent.futures.ThreadPoolExecutor}.
\end{itemize}

L'uso di una mappa statica introduce un'approssimazione di dominio: il sistema
assume che le proprietà di una classe dipendano unicamente dal suo tipo e non
dalla sequenza di salti relazionali con cui viene raggiunta. L'API di CVL, al
contrario, adotta un comportamento dinamico che mostra o nasconde determinati
vincoli in base allo storico della selezione.
...
\begin{itemize}
    \item \textbf{Abbattimento della latenza di rete:} Senza la mappa in
          memoria, ogni nodo della DFS richiederebbe un'interrogazione HTTP sincrona.
          Nelle strutture ad alta complessità combinatoria, il volume di chiamate
          aumenterebbe in modo moltiplicativo, portando i tempi di generazione da
          pochi minuti a diverse ore con un alto rischio di timeout.
    ...
    \item \textbf{Impatto marginale sulla copertura (\hyperref[obj:O1]{O1}):} La
          ...
          predicati altamente specialistici legati a combinazioni rare, preservando
          appieno la rappresentazione delle query più frequenti e rilevanti per il
          dominio clinico.
\end{itemize}
