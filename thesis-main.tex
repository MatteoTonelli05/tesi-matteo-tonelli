...
\begin{abstract}
    ...
    un Knowledge Graph interrogabile tramite SPARQL\footnote{
...
Negli ultimi anni, l'avvento dell'Intelligenza Artificiale Generativa (GenAI) e
dei \textit{Large Language Models} sta trasformando radicalmente il modo in cui
le persone interagiscono con la tecnologia, aprendo scenari inediti per la
creazione di interfacce avanzate in linguaggio naturale. Nel dominio della
sanità e della ricerca biomedica, dove la complessità delle informazioni e la
frammentazione delle basi di conoscenza rappresentano da sempre un limite
operativo, l'adozione di soluzioni basate su GenAI offre l'opportunità di
abbattere le barriere tecniche di accesso al dato. Consentire a medici, clinici
e ricercatori di interrogare i sistemi informativi sanitari ponendo semplici
domande nel proprio linguaggio naturale significa velocizzare i processi
diagnostici, facilitare l'estrazione di evidenze scientifiche e restituire al
personale sanitario un ruolo centrale e diretto nella fruizione delle
informazioni, senza la mediazione di intermediari informatici o la necessità di
apprendere linguaggi di interrogazione complessi.

In questo contesto si inserisce il presente lavoro di tesi, che affronta e
risolve l'assenza di un dataset di addestramento specifico per l'interrogazione
in linguaggio naturale del \textit{knowledge graph} del Cancer Virtual Lab. A
tale scopo, è stata progettata, sviluppata e validata una pipeline automatica a
due stadi per la generazione di dati sintetici, la cui efficacia è stata poi
dimostrata empiricamente. Il primo stadio della pipeline esplora il grafo di
conoscenza per costruire query SPARQL deterministiche e sintatticamente valide,
mentre il secondo stadio, fondato su tecniche di \textit{data augmentation},
declina tali strutture in un ampio spettro di forme conversazionali fluide e
stilisticamente varie. Per certificare in modo inconfutabile l'efficacia
metodologica della pipeline e far fronte alle limitazioni hardware dell'ambiente
di sviluppo della tesi, il dataset generato è stato impiegato per effettuare il
\textit{fine-tuning} in locale di un modello linguistico ultra-compatto.
Riuscire a infondere la complessa grammatica formale e la semantica
dell'ontologia medica in un'architettura così minimale ha costituito lo "stress
test" definitivo per la qualità del corpus sintetico, dimostrando come una
struttura di dati estremamente curata sia in grado di colmare le limitazioni
capacitive della rete neurale e ponendo basi eccellenti per il futuro impiego su
modelli di grande scala all'interno dell'infrastruttura clinica.

Il presente elaborato è organizzato in modo da guidare il lettore attraverso
tutte le fasi logiche e tecniche della ricerca. Il \cref{chap:background}
fornisce il contesto teorico, introducendo i concetti fondamentali sui grafi di
conoscenza, sul linguaggio SPARQL e sulle architetture dei modelli linguistici.
Il \cref{chap:analisi} definisce i requisiti funzionali, i vincoli di
riservatezza e gli obiettivi architetturali del progetto, mentre il
\cref{chap:progettazione} illustra nel dettaglio il design della pipeline a due
stadi. Successivamente, il \cref{chap:implementation} scende nei dettagli
operativi dello sviluppo, analizzando le sfide affrontate nella manipolazione
dell'ontologia e nella generazione stilistica. Infine, il
\cref{chap:evaluation} presenta la valutazione sperimentale dell'addestramento,
analizzando le prestazioni e le metriche di errore, prima di giungere alle
conclusioni che riassumono i traguardi raggiunti e delineano le direzioni di
sviluppo future.
...
\subsection{Sintesi Guidata da Regole e Grammatiche Formali }
...
def dfs(type_value, chain, relations_used, visited, ancestor_terminals):
    ...
    if terminals:
        routes.append({
            "chain": list(chain),
            "terminals": ancestor_terminals + [{**t, "is_final_entity": True} for t in terminals],
        })
...
quale comporta diverse chiamate di rete verso l'API di CVL. Questo processo può
...
camminando passo per passo tramite chiamate ricorsive nell'API di CVL,
...
history = [
    {
        "query_nodes": [
            {
                "predicate": {"label": "gender"},
                "values": None,
                "value_range": None
            },
            {
                "predicate": {"label": "birth date"},
                "values": None,
                "value_range": None
            }
        ]
...
def _filters_landed(sparql, chosen_values):
    ...
    \item \textbf{Parsing sintattico deterministico:} Il secondo controllo
          ...
          \begin{lstlisting}[language=Python, caption={Validazione sintattica condizionata della query SPARQL.}, label={lst:validate-sparql}]
syntax_error = (
...
[
    {
        ...
        "entities": ["Diagnosis", "TNMClassification"],
        "sample_size_per_entity": 50,
        "min_relations": 1,
        "max_relations": 3,
        "min_paths": 1,
        "max_paths": 2,
...
\subsection{Problemi di qualità riscontrati e diagnosi delle cause}
...
\subsubsection{Comportamenti anomali valutati come ammissibili.}
...
\subsection{Confronto tra generazione con e senza mascheramento}
...
\subsection{Statistiche del dataset ed analisi delle distribuzioni}
\label{sec:eval-dataset-stats}

Il corpus finale è stato suddiviso in tre insiemi disgiunti destinati ad
addestramento, validazione e test secondo la procedura di partizione per entità
descritta nella \cref{sec:impl-split}. In seguito alla generazione vi è stata
una verifica sugli identificativi univoci, attestando così l'assenza totale di
sovrapposizioni tra i tre insiemi, garantendo la misurazione della reale
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{X c c c c}
        \toprule
        \textbf{Split}      & \textbf{Record Sorgente}     & \textbf{Varianti
                                                                 Totali} &
        \textbf{Densità
            Media}     &
        \textbf{Tasso
            Fallback}                                                           \\
        \midrule
        \textbf{Train}      & 3\,443                       & 9\,976           &
        2,90 var/rec        & 0,58\% (58/9\,976)
        \\
        \textbf{Validation} & 428                          & 2\,022           &
        4,72 var/rec        & 0,35\% (7/2\,022)
        \\
        \textbf{Test}       & 428                          & 2\,023           &
        4,73 var/rec        & 0,35\% (7/2\,023)
        \\
        \midrule
        \textbf{Totale}     & \textbf{4\,299}              & \textbf{14\,021} &
        --                  & \textbf{0,51\%} (72/14\,021)
        \\
        \bottomrule
    \end{tabularx}
    \caption{Riepilogo quantitativo del dataset suddiviso per split e relative metriche di densità e fallback.}
    \label{tab:dataset-split-summary}
\end{table}

Come evidenziato nella \cref{tab:dataset-split-summary}, si osserva una marcata
differenza di densità di parafrasi tra il set di addestramento e gli insiemi di
valutazione. Nel set di addestramento, il numero di varianti per record è
governato dalla formula adattiva proporzionale alla complessità dei filtri
(media di 2,90), una scelta progettuale mirata a contenere la ridondanza
complessiva senza saturare il tempo di calcolo. Al contrario, per validation e
test è stato imposto un target fisso di 5 varianti per record (raggiunto con una
media di 4,72 e 4,73), assicurando un banco di prova ricco e diversificato per
la misurazione delle prestazioni.

In tutti e tre gli insiemi, l'incidenza delle varianti non riformulate rispetto
all'originale (\textit{fallback}) si mantiene estremamente contenuta
($<0{,}6\%$), attestando la capacità del sistema di completare la generazione
senza dover ricorrere alla frase di origine.

\subsubsection{Analisi della distribuzione stilistica.}
L'assegnazione degli stili linguistici avvenga in sequenza circolare
deterministica (\texttt{itertools.cycle}); tuttavia, la distribuzione finale
delle 9\,976 varianti dell'insieme di addestramento
(\cref{fig:style-distribution}) mostra uno sbilanciamento misurabile tra i dieci
registri: da un massimo di 1\,237 occorrenze per \emph{technical\_sql} a un
minimo di 569 per \emph{generational} (rapporto di circa 2,2 a 1).

\begin{figure}[H]
    \centering
    \includegraphics[width=.85\linewidth]{figures/style-distribution.pdf}
    \caption{Distribuzione delle varianti accettate tra i dieci registri stilistici nell'insieme di addestramento.}
    \label{fig:style-distribution}
\end{figure}

Tale discrepanza nella distribuzione degli stili non deriva da un difetto di
campionamento, ma dall'azione dei filtri di validazione della pipeline.
Direttive stilistiche più vincolanti riducono lo spazio di variazione sintattica
del modello, portandolo a convergere su strutture e lessico ricorrenti e, di
conseguenza, i meccanismi di memoria anti-ripetizione (\cref{sec:impl-styles}) e
il controllo di similarità adattiva (\cref{sec:impl-validation}) intercettano
tali ridondanze scartando un numero maggiore di tentativi per garantire la
varietà del dataset.

\subsection{Metodologia}

La valutazione del modello ha seguito un approccio strutturato in tre componenti
principali:

\begin{enumerate}
    \item \textbf{Scelta e addestramento del modello:} È stato selezionato
          \texttt{Gemma 3 270M}, un modello ultraleggero che garantisce l'esecuzione
          in locale (rispettando \hyperref[vinc:V2]{V2} e \hyperref[vinc:V3]{V3}). Il modello è
          stato addestrato tramite la tecnica LoRA (\cref{sec:peft-lora}), aggiornando
          solamente lo 0,5\% dei parametri totali per contenere i costi
          computazionali.

    \item \textbf{Definizione della metrica:} Le prestazioni sono state misurate
          calcolando l'\textbf{Exact Match (EM)}. Questa metrica indica la percentuale
          di query SPARQL generate che risultano identiche, carattere per carattere,
          alla query target di riferimento.

    \item \textbf{Allestimento dei dati di test:} La valutazione è stata
          condotta su due insiemi separati da 100 esempi ciascuno.
          \begin{enumerate}
              \item \emph{trainstyle-test} (\textit{in-distribution}), utilizza gli
                    stessi registri stilistici visti in addestramento per verificare
                    l'apprendimento delle regole base
              \item \emph{claude-test}
                    (\textit{out-of-distribution}), utilizza formulazioni generate da un modello
                    esterno (\textit{Claude 3.5 Sonnet}) per testare la capacità del sistema di
                    comprendere frasi e costrutti linguistici del tutto nuovi
          \end{enumerate}
\end{enumerate}

\subsection{Selezione del checkpoint e prevenzione dell'overfitting}

Nelle architetture di tipo Text-to-SPARQL, il numero di iterazioni di
addestramento gioca un ruolo critico; un addestramento insufficiente non
garantisce la memorizzazione dello schema, mentre un'esposizione prolungata
conduce rapidamente al collasso sui pattern di input (\textit{overfitting}).

Per identificare il regime ottimale, è stato condotto un addestramento esteso su
15\,000 iterazioni con monitoraggio continuo della \textit{validation loss}
(\cref{fig:loss-curve}).

\begin{figure}[H]
    \centering
    \includegraphics[width=.85\linewidth]{figures/loss-curve.pdf}
    \caption{Andamento della train e validation loss durante le 15\,000 iterazioni.}
    \label{fig:loss-curve}
\end{figure}

Invece di adottare il checkpoint finale (iterazione 15\,000), la selezione
metodologica è ricaduta rigorosamente sul punto di minimo della perdita di
validazione, individuato all'iterazione 7\,000 (usando dei checkpoint ogni 500
iterazioni). Questa scelta ha prodotto una svolta determinante nelle
prestazioni, portando l'Exact Match sull'insieme in-distribution dal
\textbf{15\%} del checkpoint finale al \textbf{46\%} del checkpoint ottimale
(visibile in \cref{tab:finetuning-comparison-table}), dimostrando come una
tempestiva interruzione dell'addestramento (\textit{early stopping}) sia
fondamentale per preservare la capacità di astrazione della rete.

\subsection{Confronto tra le configurazioni di addestramento}

I risultati ottenuti confermano che la combinazione tra una generazione dati
rigorosa e una strategia di fine-tuning ad alta efficienza parametrica consente
a un modello di soli 270M parametri di raggiungere una discreta accuratezza.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{X c c c}
        \toprule
        \textbf{Configurazione di fine-tuning} & \textbf{Iterazioni}      &
        \textbf{EM (In-Distr.)}                & \textbf{EM (Out-Distr.)}   \\
        \midrule
        LoRA Baseline (Short Run)              & 4\,000                   &
        13,0\%                                 & 4,0\%                      \\
        Full Fine-Tuning                       & 6\,000                   &
        18,0\%                                 & 5,0\%                      \\
        LoRA Final Checkpoint                  & 15\,000                  &
        15,0\%                                 & 7,0\%                      \\
        \textbf{LoRA Optimal (Min Val Loss)}   & \textbf{7\,000}          &
        \textbf{46,0\%}                        & \textbf{14,0\%}            \\
        \bottomrule
    \end{tabularx}
    \caption{Confronto comparativo dell'Exact Match (EM) tra le diverse configurazioni di addestramento su benchmark in-distribution ed out-of-distribution.}
    \label{tab:finetuning-comparison-table}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=.85\linewidth]{figures/finetuning-comparison.png}
    \caption{Confronto dell'Exact Match sulle configurazioni testate.}
    \label{fig:finetuning-comparison}
\end{figure}

Come sintetizzato nella \cref{tab:finetuning-comparison-table}, l'approccio
basato su LoRA all'iterazione 7\,000 supera nettamente il Full Fine-Tuning
condotto a parità di risorse (46\% contro 18\%). Il motivo di tale divario
risiede nella tendenza del Full Fine-Tuning a destabilizzare le rappresentazioni
latenti pre-addestrate del modello (\textit{catastrophic forgetting}), mentre le
matrici a basso rango di LoRA preservano le capacità grammaticali generali
adattando unicamente l'interfaccia verso la sintassi SPARQL.

Sull'insieme out-of-distribution (\emph{claude-test}), i tassi di exact match
risultano inferiori (4\% - 7\%), evidenziando la necessità di integrare in
futuro un modulo di allineamento stilistico ad ampio raggio per domande
formulate da sistemi terzi.

\subsection{Tassonomia e analisi diagnostica degli errori}

L'analisi qualitativa sui 54 casi di scarto registrati dal checkpoint ottimale
(iterazione 7\,000) offre un riscontro importante sulla solidità del sistema
(\cref{fig:error-taxonomy}).

\begin{figure}[H]
    \centering
    \includegraphics[width=.75\linewidth]{figures/error-taxonomy.pdf}
    \caption{Distribuzione percentuale delle 54 predizioni non corrispondenti per tipologia di errore sul checkpoint ottimale (iterazione 7\,000).}
    \label{fig:error-taxonomy}
\end{figure}

I fallimenti sono stati categorizzati in tre tipologie strutturali:

\begin{enumerate}
    \item \textbf{Deviazione ontologica del percorso (\textbf{63\%}, 34 casi):}
          Il modello genera una query sintatticamente perfetta, ma impiega un
          predicato o una classe adiacente errata (es. inserimento arbitrario
          della sottoclasse \texttt{DrinkerPatient} in luogo del nodo generico
          \texttt{Patient}).
    \item \textbf{Mancata corrispondenza dei valori letterali (\textbf{24\%}, 13
              casi):} La struttura di grafo e i predicati risultano esatti, ma
          uno dei valori di filtro differisce dal target di riferimento.
    \item \textbf{Scambio di codici di dominio (\textbf{13\%}, 7 casi):} La
          query individua la corretta classe, ma confonde due identificativi
          controllati specifici (es. inversione di due codici di patologia
          ICD-10 entrambi plausibili nel contesto).
\end{enumerate}

Il dato più rilevante emerso dall'analisi è che il \textbf{100\% delle query
    generate è sintatticamente valido}. In nessun caso il modello ha prodotto
parentesi spaiate, keyword SPARQL errate o violazioni di grammatica. Ciò
dimostra che la pipeline di addestramento ha insegnato con successo la
struttura rigida del linguaggio di interrogazione, riducendo l'incertezza
residua a una semplice discriminazione tra entità ontologiche contigue.
...
\begin{itemize}
    \item \textbf{Confusione tra acronimi clinici simili}
          (\cref{sec:eval-quality-issues}). Una possibile
    ...
    \item \textbf{Allucinazione di codici ICD-10}
          (\cref{sec:eval-quality-issues}). Il difetto specifico
          ...
          verifica rispetto ad una tabella dei codici validi prima di accettare qualunque
    ...
    \item \textbf{Valore letterale "nan" come unità di misura}
          (\cref{sec:eval-quality-issues}). Una correzione
...
Oltre alla risoluzione dei problemi di qualità appena discussi, si possono
proporre alcune direzioni di sviluppo più ampie, emerse dai limiti riscontrati
soprattutto nel corso della valutazione.

\begin{itemize}
    \item \textbf{Valutazione a livello di esecuzione}. La metrica di
          valutazione impiegata in questo lavoro, l'exact match, richiede una
          corrispondenza carattere per carattere tra la query generata e quella
          di riferimento. Una query semanticamente equivalente ma formulata in
          modo sintatticamente diverso, ad esempio con clausole in un ordine
          differente, viene quindi conteggiata come errore anche quando
          produrrebbe lo stesso risultato se eseguita sul knowledge graph. Una
          valutazione complementare, che esegua le query generate sull'endpoint
          di CVL e ne confronti i risultati restituiti anziché il solo testo,
          fornirebbe una stima più accurata delle prestazioni reali del modello.
    \item \textbf{Altre tecniche o modelli di adattamento}. Questo lavoro ha
          confrontato LoRA con il full fine-tuning, ma non ha testato QLoRA,
          nonostante fosse introdotta in \cref{sec:peft-lora} come tecnica in
          grado di ridurre ulteriormente l'occupazione di memoria rispetto a
          LoRA; allo stesso modo, non sono stati testati modelli di base diversi
          da \texttt{Gemma3:270M}, di dimensioni maggiori o architettura
    ...
    \item \textbf{Estenzione alla lingua
              italiana.} L'attuale pipeline produce varianti conversazionali
          esclusivamente in lingua inglese, in allineamento con la nomenclatura
          dell'ontologia di base. Uno sviluppo futuro di grande impatto applicativo
          consisterebbe nell'estendere il modulo di \textit{augmentation} per generare
          e validare registri stilistici direttamente in italiano. La creazione di un
          corpus di addestramento localizzato abiliterebbe il \textit{fine-tuning} di
          modelli capaci di tradurre interrogazioni mediche formulate nativamente in
          italiano in query SPARQL, abbattendo le barriere linguistiche per gli utenti
          finali e facilitando l'adozione del sistema da parte del personale clinico
          nel contesto nazionale.
\end{itemize}

\subsection{Conclusioni}

Questo lavoro di tesi ha affrontato e risolto l'assenza di un dataset di
addestramento specifico per l'interrogazione in linguaggio naturale del
\textit{knowledge graph} del Cancer Virtual Lab. A tale scopo, è stata
progettata, sviluppata e validata una pipeline automatica a due stadi per la
generazione dei dati, la cui efficacia è stata dimostrata empiricamente
attraverso il \textit{fine-tuning} di un modello linguistico ultraleggero.

I quattro obiettivi architetturali stabiliti in fase di analisi
(\cref{chap:analisi}) sono stati pienamente soddisfatti:

\begin{itemize}
    \item \textbf{\hyperref[obj:O1]{O1 - Copertura selettiva ed estendibilità}:}
          garantita dall'introduzione di un sistema a \textit{tier}, che mappa
          con successo le entità ontologiche selezionate con facilità. Il
          sistema risulta nativamente estendibile a nuove classi tramite
          configurazioni esterne, azzerando la necessità di interventi sul
          codice sorgente (\cref{sec:impl-tier-config}).
    \item \textbf{\hyperref[obj:O2]{O2 - Varietà linguistica autentica}:}
          ottenuta mediante un modulo di \textit{augmentation} capace di
          declinare ogni interrogazione in dieci registri stilistici differenti,
          mantenendo un tasso di successo superiore al 97\% persino nelle
          configurazioni di mascheramento più restrittive
          (\cref{sec:eval-masking-comparison}).
    \item \textbf{\hyperref[obj:O3]{O3 - Tracciabilità completa}:} assicurata
          dalla propagazione di un identificativo univoco di provenienza lungo tutta
          la pipeline, questo meccanismo ha permesso di certificare matematicamente
          l'assenza di \textit{data leakage} tra gli insiemi di \textit{train},
          \textit{validation} e \textit{test} (\cref{sec:eval-dataset-stats}).
    \item \textbf{\hyperref[obj:O4]{O4 - Volume di dati adeguato}:} validato non
          solo dalla produzione massiva di 14\,021 varianti, ma soprattutto
          dall'efficacia pratica del dataset, rivelatosi determinante per
          l'addestramento e il raggiungimento di risultati quantificabili sul modello
          linguistico finale.
\end{itemize}

Sotto il profilo empirico, la valutazione del modello ha restituito risultati di
assoluto rilievo. L'applicazione della tecnica LoRA su un'architettura
ultra-compatta di soli 270 milioni di parametri ha permesso di raggiungere un
\textit{Exact Match} del 46\% su un benchmark in stile coerente con i dati di
addestramento.

Ancora più rilevante è l'esito dell'analisi degli errori, in cui in nessun caso
il modello ha generato query sintatticamente invalide. Possiamo dunque dedurre
che la rete ha interiorizzato perfettamente la complessa grammatica SPARQL,
confinando le incertezze residue alla sola discriminazione semantica tra entità
o valori ontologici simili, escludendo così derive allucinatorie o comportamenti
arbitrari.

Infine, il rigoroso approccio iterativo adottato ha consentito di identificare,
diagnosticare e documentare con precisione cinque anomalie qualitative nel
dataset generato (\cref{sec:eval-quality-issues}). Avendo ricondotto ciascuna
casistica alla rispettiva radice tecnica, due di queste criticità sono state
risolte in modo definitivo all'interno di questo lavoro, mentre per le restanti
tre è stato tracciato un chiaro percorso di risoluzione per gli sviluppi futuri.
Parallelamente, un'analisi approfondita ha permesso di riqualificare due
comportamenti apparentemente anomali come variazioni fisiologiche e legittime.

In prospettiva, i risultati di questa ricerca dimostrano che l'uso di modelli
ultra-compatti su dati clinici, imposto dai vincoli di privacy, non
rappresenta un limite architetturale invalicabile. Se supportati da dati
sintetici strutturati di altissima qualità e da metodologie di adattamento
mirate, questi modelli possono superare la semplice validazione sintattica,
arrivando a interrogare con successo la complessa semantica di un dominio medico
specialistico.
