...
\begin{itemize}
    ...
    \item \textbf{Knowledge Graph, RDF e Strumenti SPARQL
          ...
          Python \texttt{rdflib}) utilizzati per la validazione sintattica
          deterministica delle query.
    ...
    \item \textbf{Architettura Software e Pattern
          ...
          \texttt{Streamlit} per la prototipazione di interfacce interattive
    ...
    \item \textbf{Fine-Tuning di Modelli Linguistici nel Dominio Clinico
          ...
          ultra-compatti come Gemma 3 270M per l'esecuzione locale su dati
          sensibili e descrive l'adozione del mascheramento della loss sui
          token di input (\textit{Completion-Only Data Collator}).
\end{itemize}
...
approccio utilizza grammatiche libere da contesto o alberi di derivazione logica
per combinare ricorsivamente frammenti di domande e vincoli formali.
...
\begin{itemize}
    ...
    \item \textit{Assenza di controlli strutturali:} l'assenza di un
          collegamento deterministico tra la query SPARQL e il prompt generato rende
          impossibile garantire la correttezza semantica senza un'accurata validazione
          esterna.
\end{itemize}
...
Per tali ragioni, il presente lavoro adotta una strategia ibrida e controllata:
un generatore combinatorio strutturato basato sullo schema del grafo (per
garantire la correttezza formale e la copertura mirata delle 45 entità)
integrato con un secondo stadio di augmentation linguistica guidata da LLM, ma
rigorosamente vincolata da un meccanismo di mascheramento dei valori letterali.
...
I tre sotto-problemi individuati nell'analisi vengono affrontati da componenti
distinti della pipeline. \hyperref[prob:P1]{P1} è affrontato attraverso il
generatore parametrico di query, che consente di controllare lo spazio delle
configurazioni esplorate. \hyperref[prob:P2]{P2} è affrontato attraverso la fase
di augmentation linguistica, progettata per produrre formulazioni diverse a
partire dalla medesima query. \hyperref[prob:P3]{P3} richiede invece una fase di
validazione, che verifica gli output prodotti dalle fasi precedenti prima del
loro inserimento nel dataset.
...
questa scelta in una struttura di query, poi decidere con quali valori concreti
riempirla, poi trasformare il tutto in un testo in linguaggio naturale, e infine
verificare che il risultato ottenuto sia davvero corretto. Concatenare tutti
questi passaggi in un'unica procedura indifferenziata renderebbe il codice
difficile da comprendere, da testare e da modificare, perché un cambiamento
riguardante anche un solo aspetto, per esempio il modo in cui si sceglie un
valore, rischierebbe di intrecciarsi con la logica di tutti gli altri.

Il generatore struttura quindi questo processo come una pipeline di fasi ben
definite, ciascuna responsabile di una trasformazione specifica, che riceve in
ingresso il risultato della fase precedente e produce l'input per quella
successiva.

Questa scomposizione rende ciascuna fase comprensibile e verificabile in
isolamento, senza dover ragionare sull'intero processo per capire o modificare
una singola trasformazione. Apre inoltre la strada, in fase di implementazione,
a una corrispondenza diretta tra ciascuna fase concettuale qui definita e il
componente software responsabile di realizzarla (\cref{sec:impl-mixin}).
...
capitolo di analisi. Trattando le quarantacinque classi come equivalenti, il
sistema avrebbe allocato un budget di generazione identico a ciascuna,
indipendentemente dalla sua centralità nel dominio clinico. Classi
concettualmente centrali, quali Patient, Diagnosis e Therapy, avrebbero ricevuto
una rappresentazione insufficiente rispetto alla frequenza con cui compaiono
nell'uso reale, mentre classi periferiche e altamente granulari, quali le
singole categorie della classificazione TNM, avrebbero prodotto un numero di
esempi ridondante rispetto alla loro effettiva rilevanza. Il risultato atteso di
un simile sbilanciamento non sarebbe stato un dataset semplicemente meno
efficiente, ma un modello allenato su di esso incline a generalizzare peggio
proprio sulle categorie di domande più frequenti nell'uso previsto del sistema.
...
l'applicazione di una formula matematica definita anticipatamente. Lo stesso
principio di calibrazione empirica viene applicato al sistema dei quattro
parametri descritto nella sezione seguente.
...
complessità applicati a ciascuna sono stati scelti con l'obiettivo di riflettere
le tipologie di domande ritenute più rilevanti nel contesto del Cancer Virtual
Lab, privilegiando la rilevanza del dominio rispetto alla massimizzazione della
copertura formale del grafo.
...
precedenza. La proprietà è quindi garantita a livello parametrico; l'assenza di
duplicati a livello di query è invece una proprietà che dipende dal modo in cui
le configurazioni vengono trasformate in query e, se necessario, può essere
...
nuove porzioni al dataset non richiede una riprogettazione del generatore, ma la
definizione di nuovi preset i cui intervalli siano scelti in relazione a quelli
già utilizzati. La procedura può essere applicata ripetutamente, mantenendo
traccia dello spazio parametrico già esplorato e introducendo nuove
...
Il problema affrontato da questo stadio è duplice: da un lato produrre varie
riformulazioni linguisticamente a partire da uno stesso prompt, dall'altro
...
rischio già discusso in \hyperref[rf:RF3]{RF3} e \hyperref[rnf:RNF3]{RNF3},
ovvero una riformulazione apparentemente valida ma silenziosamente scorretta
...
Il numero di varianti richieste per ciascun record non è inoltre fissato in modo
uniforme, ma proporzionato al numero di valori mascherati nel record: un prompt
con pochi valori offre in pratica poco margine di riformulazione prima di
esaurire le combinazioni naturali, per cui imporre lo stesso numero di varianti
richieste a un record semplice e a uno complesso avrebbe prodotto, nel primo
caso, un'alta proporzione di tentativi rifiutati per eccessiva similarità.

\subsection{Varietà stilistica}

Il solo mascheramento non garantisce, di per sé, la varietà linguistica
richiesta da \hyperref[obj:O2]{O2}: un modello lasciato libero di riformulare
tende a convergere su un piccolo numero di strutture sintattiche ricorrenti. Il
...
interrogazione a database, una clinico-formale, una colloquiale, una statistica
orientata al conteggio), esplicitamente descritti al modello a ogni chiamata.
...
termine delle formulazioni e delle parole di apertura usate più di recente: un
nuovo candidato che ripropone un'apertura già usata di recente per lo stesso
stile viene rifiutato dalla validazione allo stesso modo di una riformulazione
troppo simile a livello di intero testo, estendendo così il controllo di varietà
anche alla struttura delle prime parole della frase, non solo al suo contenuto
complessivo.

Il sistema conserva infine, per ciascun record, anche una modalità alternativa
priva di mascheramento, che riproduce il comportamento originario a chiamata
singola sull'intero testo. Questa modalità non costituisce un secondo percorso
di produzione, ma un termine di paragone esplicito, i cui risultati sono
discussi assieme a quelli della modalità con mascheramento nel capitolo di
valutazione.
...
\emph{mixin}\footnote{Un mixin è una classe pensata per essere combinata con
    ...
    senza costituire di per sé una gerarchia di tipi completa.}, uno per ciascuna
fase della pipeline. Questa struttura permette di mantenere la corrispondenza
diretta tra fase concettuale e componente software già motivata in fase di
design, con un'unica interfaccia per l'intero processo di generazione: le
diverse fasi possono infatti essere modificate o ampliate intervenendo
principalmente sul mixin che ne implementa la relativa responsabilità, senza
dover toccare gli altri. La composizione della classe principale è riportata nel
...
design, incapsulando le chiamate di rete o coordinando la logica di più fasi
quando queste sono strettamente collegate tra loro:
...
run. Il secondo è lo stato dell'interfaccia Streamlit, dal quale viene letto il
valore di \texttt{session\_state["abort\_generation"]}. Quando l'utente richiede
l'interruzione, il metodo imposta l'evento interno; le chiamate successive
possono quindi rilevare direttamente tale stato senza dover interrogare
nuovamente l'interfaccia.
...
completa delle proprietà e delle relazioni disponibili per ciascuna entità dello
schema, interrogando l'API interna della piattaforma CVL. Questa mappa viene
costruita una sola volta e riutilizzata per l'intera esecuzione, evitando di
ripetere le stesse chiamate di rete ogni volta che un'entità già scoperta viene
esplorata di nuovo. Il dettaglio implementativo di come l'API venga interrogata
non è discusso in questa sede, trattandosi di un'interfaccia della piattaforma
CVL e non di un contributo originale di questa tesi; qui rileva solo il
risultato che il generatore ne ricava, riportato in forma semplificata nel
\cref{lst:graph-map}: per ogni entità, l'insieme delle sue proprietà filtrabili
(\texttt{properties}) e delle entità verso cui può proseguire una relazione
(\texttt{relations}).
...
graph_map = { "properties": { "Patient": [ { "predicate": {"value":
    "has_gender", "label": "gender"}, "datatype": "string", "n_values": 2,
    "values": [ {"value": "F", "label": "Female"}, {"value": "M", "label":
    "Male"}, ], }, ], }, "relations": { "Patient": [ {"predicate": {"value":
    "has_diagnosis", "label": "diagnosis"}, "target": {"value": "Diagnosis",
    "label": "Diagnosis"}}, ], }, }
\end{lstlisting}

Ogni proprietà filtrabile porta con sé, oltre al predicato che la identifica, il
tipo di dato, il numero di valori distinti disponibili (\texttt{n\_values}) e i
valori stessi, già raccolti in questa fase (fino al tetto
\texttt{MAX\_STORED\_VALUES} descritto più sotto); ogni relazione porta invece
il predicato usato per il salto e l'entità di destinazione (\texttt{target}).

La scelta di indicizzare entrambe le sezioni per nome di entità, invece di usare
una singola lista di record, non è casuale. La mappa viene infatti interrogata
ripetutamente durante la scoperta delle route (\cref{sec:impl-routes}), una
volta per ciascuna entità di volta in volta esplorata, e un dizionario permette
di recuperare le proprietà o le relazioni di una data entità in tempo costante,
invece di dover scorrere l'intera struttura ogni volta.

Costruire la mappa una volta sola, prima di iniziare qualunque generazione,
comporta anche un compromesso più ampio. Quando viene aperta, ogni entità viene
interrogata come punto di partenza isolato, con una storia di selezione vuota,
indipendentemente dal fatto che nell'esplorazione vera delle route quella stessa
entità venga poi raggiunta attraverso un percorso diretto o attraverso più salti
relazionali. La mappa assume quindi che le proprietà e le relazioni disponibili
per un'entità dipendano solo dal suo tipo, non dal percorso concreto con cui la
si raggiunge, a differenza dell'API del CVL, che può restituire relazioni
diverse a seconda della storia di selezione accumulata fino a quel punto. Questo
costo viene accettato deliberatamente, perché l'alternativa, interrogare l'API
da capo ogni volta che una qualunque entità viene esplorata, è proprio il collo
di bottiglia che si vuole evitare.

Per ridurre il tempo complessivo di questa fase, le interrogazioni relative alle
diverse opzioni di una stessa entità vengono eseguite in parallelo, fino a un
massimo configurabile per volta (\texttt{MAX\_DISCOVERY\_WORKERS}), tramite un
pool di thread; i valori restituiti per ciascuna proprietà filtrabile vengono
conservati fino a un tetto massimo (\texttt{MAX\_STORED\_VALUES}), per non
tenere in memoria un elenco potenzialmente enorme quando un campo ammette
moltissimi valori distinti.
...
combinazioni così stimate per costruire ogni singolo record. I due passi sono di
natura diversa: la stima è un conteggio puramente matematico, espresso di
seguito con delle formule; il campionamento è invece un algoritmo vero e
proprio, mostrato più avanti in codice.
...
lì veniva fatto una volta per ogni proprietà isolata, per costruire la mappa,
mentre qui viene ripetuto per intero a ogni singolo record. Durante questa fase
di costruzione online sono inoltre presenti alcuni meccanismi di resilienza,
...
rivelati necessari nel corso dello sviluppo e vengono descritti di seguito.
...
\begin{enumerate}
    \item Il primo verifica che la query restituita dall'API contenga davvero
          una clausola \texttt{FILTER} o \texttt{VALUES}, oppure almeno uno dei
          valori letterali scelti. Questo controllo esiste perché l'API
          potrebbe, in linea di principio, restituire una query sintatticamente
          valida ma priva di ogni vincolo reale, per esempio se un valore non
          venisse tradotto correttamente in un filtro effettivo: una query del
          genere risulterebbe innocua da eseguire, ma completamente slegata dal
          prompt che dovrebbe rappresentare, e finirebbe nel dataset come
          esempio scorretto senza che nulla lo segnali. Se nessuna delle due
          condizioni è soddisfatta, il record viene scartato con il motivo
...
def _filters_landed(sparql, chosen_values): if "FILTER" in sparql or "VALUES" in
    sparql: return True return any(v in sparql for v in chosen_values)
          \end{lstlisting}

    \item Il secondo verifica la validità sintattica della query tramite un vero
          parser SPARQL, la libreria
          \texttt{rdflib}\footnote{\url{https://rdflib.readthedocs.io/}}. Serve
          a intercettare query malformate prima che finiscano nel dataset: un
          esempio di addestramento con una query che non si può nemmeno
          interpretare sarebbe peggio che inutile, perché insegnerebbe al
          modello una sintassi scorretta. Il controllo è condizionato dal
          parametro \texttt{validate\_sparql} (\cref{lst:common-config}),
          impostato a vero nei run di produzione; ogni query che non superava il
          parsing veniva quindi scartata per davvero, con il motivo esplicito
...
syntax_error = ( validate.sparql_error(sparql) if
    self.config.get("validate_sparql", True) else None )
          \end{lstlisting}
    ...
    \item Il terzo verifica che la coppia non sia un duplicato, tenendo in
          ...
          calcolati su ogni coppia prompt-query già accettata: un nuovo prompt
          viene scartato se è già in quell'insieme, una nuova coppia se il suo
          hash coincide con uno già registrato. Senza questo controllo, il
          campionamento potrebbe scegliere per caso, in record diversi, la
          stessa combinazione di route e valori, riducendo la varietà effettiva
          senza che il conteggio totale dei record se ne accorga. In caso di
          prompt duplicato, il sistema ritenta con nuovi valori scelti a caso
          fino a 4 volte (\texttt{VALUE\_REROLLS}) prima di arrendersi.
\end{enumerate}

Solo le coppie che superano tutti e tre i controlli vengono accumulate in un
buffer e scritte su disco a gruppi di 50 (\texttt{BUFFER\_SIZE}).

\subsection{Generazione manuale a entità singola}
\label{sec:impl-manual}

Tutto quanto descritto finora in questo capitolo, dalla scoperta della mappa
fino ai controlli di qualità, produce record per \emph{una} entità alla volta,
data una combinazione specifica di route e proprietà. Il modo più diretto per
eseguire questo motore è farlo a mano, entità per entità, tramite un pannello
dell'interfaccia Streamlit, organizzato in cinque sezioni.
...
%\caption{Mockup semplificato del pannello di generazione manuale: selezione
%dell'entità, i quattro slider di configurazione, e il riscontro immediato sulle
%route effettivamente scoperte. Non sono raffigurate le sezioni \emph{Generation
%Mode} e \emph{History building}, i cui parametri sono già discussi altrove in
%questo capitolo.} \label{fig:manual-mockup} \end{figure}
...
La procedura manuale appena descritta agisce su una sola entità alla volta, ma
entità diverse dello schema possono richiedere impostazioni diverse tra loro a
seconda di quanto siano rilevanti per il compito, di quanto siano profondamente
connesse al resto del grafo, o di quante variazioni valga la pena
rappresentarne.

Il generatore permette di raggruppare più entità sotto uno stesso insieme di
parametri condivisi, un \emph{tier}, così da poter personalizzare queste
impostazioni gruppo per gruppo invece di doverle applicare in modo uniforme a
tutto lo schema, e di poter riutilizzare la stessa configurazione, o adattarla
facilmente, in generazioni future. Per ciascun tier configurato, per ciascuna
entità elencata al suo interno, viene eseguito lo stesso motore di generazione
già descritto nelle sottosezioni precedenti, con i parametri propri di quel tier
(gli stessi utilizzati nella \cref{sec:impl-manual}) e un numero massimo di
record da generare per quell'entità (\texttt{sample\_size\_per\_entity}). I
record delle singole entità confluiscono infine in un unico file
(\cref{sec:impl-split}).

Questa configurazione esterna è esposta, sul lato dell'interfaccia Streamlit,
tramite un pannello dedicato che permette di caricare il file e ne mostra a
schermo un riepilogo, con il numero di gruppi e di entità coinvolte, prima di
abilitare il pulsante di avvio; l'avanzamento della generazione viene poi
tradotto in una barra di progresso calcolata sul numero di entità completate
rispetto al totale. L'output finale viene poi reso disponibile direttamente come
file scaricabile al termine del processo, eliminando così la necessità di un
...
\begin{enumerate}
    \item \textbf{Alterazione e perdita di valori clinici
              (\textit{hallucination}):} Il modello tendeva a modificare o
          omettere identificativi, date, codici clinici e valori booleani,
    ...
    \item \textbf{Instabilità al crescere della complessità:} All'aumentare del
          ...
          qualità della parafrasi degradava rapidamente, portando a omissioni di
          intere clausole.
...
Il primo passo di ogni record è la sostituzione di ciascun valore letterale con
un token opaco (\cref{lst:mask}).
...
def generate_placeholder_tokens(n): tokens = set() while len(tokens) < n:
    tokens.add("[VAL_" + "".join(random.choices(TOKEN_ALPHABET, k=4)) + "]")
    return list(tokens)

def mask(text, spans): spans = sorted(spans) tokens =
    generate_placeholder_tokens(len(spans)) mapping, pieces, cursor = {}, [], 0
    for (start, end), placeholder in zip(spans, tokens):
    pieces.append(text[cursor:start]) mapping[placeholder] = text[start:end]
    pieces.append(placeholder) cursor = end pieces.append(text[cursor:]) return
    "".join(pieces), mapping
\end{lstlisting}

L'alfabeto dei token esclude deliberatamente i caratteri facilmente confondibili
tra loro a colpo d'occhio (\texttt{I}, \texttt{L}, \texttt{O}, \texttt{0},
\texttt{1}); i token vengono generati dentro un insieme (\texttt{set}) finché
non se ne hanno abbastanza di univoci, escludendo così collisioni tra due
placeholder nello stesso record. La funzione \texttt{mask} opera direttamente
sugli span di posizione prodotti dal generatore (\cref{sec:impl-phrasing}),
tagliando il testo esattamente su quelle coordinate senza alcuna ricerca a
posteriori. Due funzioni di regex ricorrono in tutto il modulo per riconoscere i
placeholder, con scopi diversi: una versione rigida (\texttt{VAL\_RE}), che
combacia solo con il formato esatto appena mostrato, usata per estrarre i
placeholder autentici da un testo; e una versione permissiva
(\texttt{LOOSE\_VAL\_RE}), che riconosce anche forme storpiate o inventate,
usata in fase di validazione per intercettare allucinazioni del modello
(\cref{sec:impl-validation}).

\subsection{Riformulazione booleana e suggerimenti naturali}
\label{sec:impl-boolean-hints}

Non tutti i placeholder attraversano l'intera pipeline di generazione: i valori
booleani vengono eliminati subito, prima ancora di contattare il modello
(\cref{lst:boolean}).
...
def simplify_boolean_phrasing(text, mapping): new_mapping = dict(mapping) for
    placeholder, value in mapping.items(): if value.lower() not in {"true",
    "false"}: continue is_true = value.lower() == "true" repl = r"with \1" if
    is_true else r"without \1" esc = re.escape(placeholder) stop =
    r"(?:with|which|and)" which_re = re.compile(rf"\bwhich ((?:(?!{stop}\b)\w+
    ?){{1,5}}) is {esc}", re.IGNORECASE) with_re = re.compile(rf"\bwith
    ((?:(?!{stop}\b)\w+ ?){{1,5}}) {esc}", re.IGNORECASE) if
    which_re.search(text): text = which_re.sub(repl, text) del
    new_mapping[placeholder] elif with_re.search(text): text = with_re.sub(repl,
    text) del new_mapping[placeholder] return text, new_mapping
\end{lstlisting}
...
deterministiche si applica, viene interrogato il modello per un sinonimo d'uso
comune. Questo suggerimento viene usato, con una probabilità configurabile, al
momento del ripristino finale dei valori (\cref{sec:impl-validation}), e in modo
differente nella modalità priva di mascheramento (\cref{sec:impl-nomask}).
...
def split_into_chunks(masked_text, mapping, max_values_per_chunk): clauses =
    _protect_between(masked_text).split(CLAUSE_SEP) chunks = [[]] count_in_chunk
    = 0 for clause in clauses: n_values = len(VAL_RE.findall(clause)) if
    count_in_chunk > 0 and count_in_chunk + n_values > max_values_per_chunk:
    chunks.append([]) count_in_chunk = 0 chunks[-1].append(clause)
    count_in_chunk += n_values return [_restore_between(CLAUSE_SEP.join(c)) for
    c in chunks if c]
\end{lstlisting}
...
temporaneamente l'\emph{"and"} interno a un intervallo \emph{"between X and Y"}
con un segnaposto interno che non compare mai per caso in un testo inglese (il
carattere NUL), impedendo che un intervallo venga spezzato a metà tra due chunk
diversi. Ogni record con un numero di valori pari o inferiore alla soglia
produce un solo chunk, e non attraversa quindi alcuna suddivisione: nel dataset
supplementare da 1331 record generato con vincolo fisso a 4 proprietà per record
(\cref{sec:impl-tier-config}), l'80,6\% dei record (1073 su 1331) è rimasto in
un solo chunk, mentre il restante 19,4\% (258 record) è stato diviso in due;
nessun record ha richiesto più di due chunk, nemmeno quello con più valori in
assoluto (20 valori, divisi in due chunk da 11 e 9).

\subsection{Generazione iterativa: stile, retry e fallback}
\label{sec:impl-generation}

Ogni chunk, e successivamente ogni variante completa, viene generato tramite un
ciclo di tentativi con feedback esplicito sul motivo di un eventuale rifiuto e
temperatura crescente (\cref{lst:generate-piece}).
...
def generate_piece(model, piece_text, piece_mapping, style_desc, max_attempts,
                    base_temp, max_temp, avoid_history, indent, label,
                    avoid_text=None, avoid_reason=None, as_fragment=False,
                    language="English"): total_elapsed = 0.0 for attempt in
                    range(max_attempts): temperature =
                    temperature_for_attempt(attempt, max_attempts, base_temp,
                    max_temp) candidate, elapsed = ( paraphrase_fragment(model,
                    piece_text, piece_mapping, temperature, language,
                    avoid_text, avoid_reason) if as_fragment else
                    paraphrase_one(model, piece_text, piece_mapping, style_desc,
                    temperature, language, avoid_text, avoid_reason,
                    avoid_history if attempt == 0 else None) ) total_elapsed +=
                    elapsed reason = validation_reason(candidate, piece_mapping,
                    []) if reason is None: return candidate, total_elapsed,
                    False avoid_text, avoid_reason = candidate, reason return
                    piece_text, total_elapsed, True
\end{lstlisting}

Se il candidato viene rifiutato, il testo scartato e il motivo esatto del
rifiuto diventano il contesto per il tentativo successivo, mentre la temperatura
sale linearmente dalla base al massimo configurato
...
comportamento di ripiego. Il primo chunk di un record usa il template completo,
con l'assegnazione di stile e la cronologia delle aperture di frase recenti da
evitare; i chunk successivi usano un template più semplice \emph{"a frammento"}
(\texttt{as\_fragment=True}), senza ripetere l'assegnazione di stile, per
restare coerenti col registro già stabilito dal primo pezzo. Ogni chunk viene
validato singolarmente, ma solo sull'integrità dei placeholder: il controllo di
somiglianza, che richiede un confronto con varianti già accettate, si applica
soltanto al termine, sul testo ricomposto (\cref{sec:impl-validation}).
...
Il controllo di somiglianza usa
\texttt{difflib.SequenceMatcher}\footnote{\url{https://docs.python.org/3/library/difflib.html}}
della libreria standard, con una soglia che dipende dalla lunghezza del testo:
più permissiva (fino al 97\%) per frasi brevi, più severa (85\%) per frasi
lunghe, altrimenti testi corti verrebbero quasi sempre scartati come duplicati
per mancanza di margine di variazione lessicale. Poiché questa funzione riceve
sempre una lista vuota quando è invocata sui singoli chunk, il controllo di
somiglianza non si attiva mai a quel livello: è un ciclo su una lista vuota, non
un controllo disattivato esplicitamente.

Per la modalità priva di mascheramento (\cref{sec:impl-nomask}) esiste una
seconda funzione di validazione, più permissiva sul primo controllo: un valore
non è considerato perso se compare nella sua forma di suggerimento naturale, o
come sinonimo plausibile case-insensitive, non solo nella sua forma letterale
esatta, coerentemente col fatto che, in questa modalità, il modello vede il
valore vero e ha il permesso esplicito di riformularlo.
...
oggetto che assegna gli stili in sequenza fissa e ricorda le generazioni recenti
per stile (\cref{lst:style-tracker}).
...
class StyleTracker: def __init__(self, styles=None, recent_size=3,
    max_chars=160, opener_window=12): self._styles = styles if styles is not
    None else STYLES self._style_cycle = itertools.cycle(self._styles)
    self._recent_texts = {tag: [] for tag, _ in self._styles}
    self._opener_history = {tag: [] for tag, _ in self._styles}

    def next_style(self): return next(self._style_cycle)

    def remember(self, style_tag, text): sanitized = VAL_RE.sub("...", text)
        preview = sanitized if len(sanitized) <= self._max_chars else
        sanitized[:self._max_chars - 3] + "..." bucket =
        self._recent_texts.setdefault(style_tag, []) bucket.append(preview) del
        bucket[: -self._recent_size] openers =
        self._opener_history.setdefault(style_tag, [])
        openers.append(opener(text)) del openers[: -self._opener_window]
\end{lstlisting}
...
elevato di record. \texttt{remember} viene chiamata solo su varianti accettate,
mai su tentativi rifiutati: tiene una finestra delle ultime 3 formulazioni
complete per stile (con i placeholder ancora presenti sostituiti da
\texttt{"..."}, per non confrontare mai codici opachi come se fossero testo
naturale) e una finestra più ampia, le ultime 12, delle sole aperture di frase.
La finestra qui è più ampia perché le prime due parole di una frase sono
un'informazione più povera di una frase intera, e richiedono quindi uno storico
più esteso per intercettare ripetizioni. Le due informazioni sono tenute in due
dizionari distinti, invece che in un'unica struttura combinata, proprio perché
servono controlli diversi con finestre di lunghezza diversa: tenerle separate
evita di dover troncare artificialmente l'una alla lunghezza dell'altra.
...
def to_dict(self): slots_failed = len(self.failed_slots) yield_rate = (1 -
    (slots_failed / self.total_slots)) if self.total_slots else None return {
    "total_slots": self.total_slots, "slots_failed": slots_failed, "yield_rate":
    yield_rate, "fallback_count": self.fallback_count, "style_attempts":
    dict(self.style_attempts), "style_accepted": dict(self.style_accepted),
...
completa (non solo i tentativi per singolo chunk), non si è prodotto nulla; per
ciascuna, viene conservato anche il dettaglio di ogni tentativo fallito e il
relativo motivo. Le statistiche vengono salvate su disco dopo ogni record
processato, non solo a fine run: un'esecuzione interrotta e ripresa continua ad
accumulare sugli stessi contatori invece di ripartire da zero. Allo stesso scopo
di resilienza, l'elenco dei \texttt{source\_id} già presenti nel file di output
viene ricostruito a ogni avvio, così che i record già completati vengano saltati
in caso di ripresa dopo un'interruzione (\emph{RNF2}).
...
def generate_variant_no_mask(model, text, values, style_tag, style_desc,
                              ...
                              language="English", hints=None): # Deliberately no
                              chunking here: this mode exists to reproduce the #
                              pre-masking, single-shot behavior for direct
                              comparison, not to be a # second production path.
                              hints = hints or {} identity_mapping = {v: v for v
                              in values} value_list = ", ".join(f'"{v}"' for v
                              in values) ... reason =
                              validation_reason_with_hints(candidate, values,
                              hints, [])
\end{lstlisting}

Il commento nel codice sorgente è esplicito sul ruolo di questa modalità: non un
secondo percorso di produzione, ma un termine di paragone diretto per
...
\cref{chap:progettazione}: su 41 rifiuti registrati nel log di un run limitato a
due record, 36 (87,8\%) erano dovuti a valori letterali persi o alterati dal
modello: un esito reso strutturalmente impossibile dal mascheramento, dato che
in quella modalità il modello non ha mai accesso al valore reale durante la
riformulazione.
...
Nel corso dello sviluppo sono stati individuati cinque problemi di qualità veri
e propri, ciascuno diagnosticato fino alla causa tecnica sottostante
(\cref{fig:quality-issues}). Una proposta di risoluzione per ciascuno di essi è
discussa in \cref{chap:conclusion}. Due ulteriori comportamenti, inizialmente
considerati problematici, sono stati in seguito riconosciuti come accettabili o
addirittura desiderabili, e sono discussi separatamente al termine di questa
sezione.

\label{prob:ecog}
Un primo problema, isolato, ha riguardato la confusione tra l'acronimo clinico
ECOG, relativo a una scala di stato funzionale oncologico, e il termine
ecocardiogramma, un esame diagnostico distinto; il modello linguistico locale
impiegato per la generazione dei sinonimi ha in un'occasione sostituito il primo
con una forma testuale riconducibile al secondo. La causa è riconducibile ai
limiti di conoscenza specifica del modello linguistico di dimensioni contenute
impiegato per questo compito, che non dispone di una base affidabile di
terminologia clinica e tende a produrre un'associazione plausibile in base alla
sola somiglianza superficiale tra i due termini, piuttosto che riconoscere di
non avere l'informazione necessaria.
...
classificazione ICD-10. Il filtro di sicurezza del sistema di sinonimi, nella
sua versione iniziale, escludeva dal tentativo di generazione del sinonimo i
valori corrispondenti al pattern di un codice, riconosciuto tramite un prefisso
testuale seguito dal carattere due punti; tale riconoscimento, tuttavia, era
limitato a prefissi composti interamente da lettere, ed escludeva correttamente
prefissi quali quello della nomenclatura SNOMED, ma non il prefisso della
classificazione ICD-10, che contiene anche cifre. Come conseguenza, un medesimo
codice ICD-10 è stato tradotto, in varianti diverse dello stesso record, in nomi
di patologie non corrispondenti al codice originale e reciprocamente incoerenti
tra loro, confermando che il modello linguistico non disponeva di una conoscenza
affidabile della corrispondenza tra codice e patologia, ma produceva
un'associazione plausibile priva di fondamento verificabile, lo stesso limite
già osservato nel primo problema. Il problema ha interessato quarantasette
record del dataset; il difetto specifico nel filtro di riconoscimento del
pattern, che escludeva solo i prefissi alfabetici, è stato corretto.

Un terzo problema, riscontrato nella prima fase del lavoro di generazione
manuale delle varianti per gli insiemi di validazione e di test, ha riguardato
la produzione di testo non conforme a un registro di linguaggio naturale, quale
la riproduzione di sintassi propria di un linguaggio di interrogazione
strutturato, elenchi nella forma campo seguito da valore, e parentesi impiegate
come dispositivo strutturale anziché come parte di un valore letterale
originale. A differenza dei primi due problemi, la causa qui non è un limite del
modello linguistico, ma la scrittura manuale dei modelli di frase impiegati in
questa fase, che in alcuni casi rispecchiava abitudini di formulazione più
vicine a un'interfaccia tecnica che al linguaggio naturale. Il problema è stato
individuato tramite lettura diretta del testo prodotto e corretto riscrivendo i
modelli di frase coinvolti.

Un quarto problema ha riguardato la concatenazione di due valori data adiacenti
in un intervallo temporale, priva di un separatore testuale intermedio,
risultante in una sequenza di cifre e trattini non interpretabile senza
ambiguità da un lettore umano, poiché le date stesse contengono trattini come
parte della propria formattazione. La causa è di natura puramente
implementativa, non riconducibile ad alcun modello linguistico: il modello di
frase impiegato per costruire il testo non prevedeva l'inserimento di una
congiunzione quando due valori data si trovavano ad essere affiancati
direttamente. Il problema, riscontrato in cinquantacinque occorrenze, è stato
corretto inserendo sistematicamente una congiunzione testuale in tutti i punti
in cui ciò si verificava.

\label{prob:nan}
Un quinto problema, distinto dai precedenti poiché non riconducibile a un
difetto del sistema di generazione delle varianti, ha riguardato la presenza del
valore letterale "nan" come unità di misura in un numero limitato di record,
riscontrato in sette casi. Questo valore, tipico di una rappresentazione di dato
mancante non gestita a monte, è stato preservato correttamente dal sistema di
augmentation, che per costruzione non altera i valori letterali originali; la
causa è quindi da ricondurre alla fase di generazione del dataset strutturato,
non alla fase di augmentation linguistica.

\begin{figure}[H]
    \centering
    \includegraphics[width=.85\linewidth]{figures/quality-issues.pdf}
    \caption{Occorrenze riscontrate per ciascuno dei quattro problemi di qualità
        quantificabili (il terzo problema, individuato tramite lettura diretta
        anziché conteggio sistematico, non compare in figura).}
    \label{fig:quality-issues}
\end{figure}

Due ulteriori comportamenti, inizialmente registrati come problemi durante lo
sviluppo, meritano una lettura diversa. Il primo riguarda l'uso occasionale di
una preposizione non canonica, "at" al posto di "on", in combinazione con valori
riferiti a una linea di trattamento terapeutico, riscontrato in otto casi. Più
che un difetto da correggere, questo comportamento può essere considerato una
forma di variazione naturale del linguaggio: un parlante reale non segue sempre
con precisione le regole grammaticali, e una minima imprecisione di questo tipo
può contribuire alla varietà stilistica del testo generato piuttosto che
comprometterne la qualità.

Il secondo riguarda la presenza, in alcuni record, di due vincoli distinti sulla
medesima proprietà scalare del paziente, quale la data di decesso, con valori
incompatibili tra loro, risultante in una query logicamente priva di risultati
possibili. Il fenomeno, ricondotto a una collisione tra due percorsi relazionali
paralleli campionati indipendentemente durante la generazione, è risultato più
frequente nel secondo passaggio di generazione, con duecentonovantacinque record
su milletrecentotrentuno esaminati in quella fase. Piuttosto che un difetto da
correggere, questo comportamento riflette una condizione che si presenta anche
nell'uso reale di un sistema di interrogazione: un utente può porre, per errore
o per verifica, una domanda a cui nessun dato può corrispondere. Includere
alcuni esempi di questo tipo nel dataset non compromette la qualità
dell'addestramento, e i record interessati sono comunque stati esclusi dal
dataset finale per mantenere la maggioranza dei casi orientata a query con una
risposta effettiva.

\subsubsection{Confronto tra generazione con e senza mascheramento}
\label{sec:eval-masking-comparison}

Al fine di verificare se l'assenza di mascheramento dei valori producesse una
struttura linguistica delle varianti generate qualitativamente diversa rispetto
alla modalità principale, è stato condotto un confronto sperimentale su un
insieme di venti record di input, selezionati per rappresentare una gamma di
complessità crescente, dal caso di un singolo valore vincolato fino al caso di
dodici valori vincolati nella medesima query. Le due modalità sono state
eseguite sul medesimo insieme di record, con i medesimi parametri di
generazione.

Il numero complessivo di slot di generazione tentati è risultato identico tra le
due modalità, sessantasei, poiché determinato dalla medesima formula adattiva
basata sul numero di valori per record, indipendente dalla modalità di
mascheramento. Il tasso di successo complessivo è risultato lievemente superiore
nella modalità con mascheramento, novantotto virgola cinque percento contro
novantasette percento nella modalità senza mascheramento. La causa dei
fallimenti si è rivelata inoltre di natura diversa tra le due modalità: nella
modalità senza mascheramento, i fallimenti osservati sul medesimo record sono
stati causati da una corrispondenza testuale identica al cento percento tra
tentativi di generazione distinti, condizione non osservata nella modalità con
...
compromettendo in modo sostanziale il tasso di successo complessivo, può ridurre
la capacità del modello linguistico di produrre formulazioni distinte quando il
testo con i valori visibili offre un numero limitato di modi naturali per
esprimere la medesima richiesta.
...
sono stati testati modelli di base diversi da Gemma 3 270M, di dimensioni
maggiori o architettura differente, che potrebbero offrire una migliore capacità
di generalizzazione a scapito di un costo computazionale più elevato.
